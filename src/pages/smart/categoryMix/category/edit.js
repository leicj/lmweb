...
const MyEdit = ({ dispatch, visible, data }) => {
  ...
  const onOk = (form) => {
    form.validateFields().then(values => {
      dispatch({
        type: "category/edit", formData: values, _id: values._id, cb: () => {
          dispatch({ type: "category/reditData", editData: { data: {}, visible: false } });
  ...
  React.useEffect(() => {
    ...
    !_id && form.setFieldsValue({
      _id: data._id, name: data.name, status: data.status, struct: data.struct,
    });
  });
  return (
    <Modal title="编辑" open={visible} onOk={() => onOk(form)} onCancel={() => onCancel(form)} style={{ minWidth: 600 }}>
      <Form form={form} name="编辑对策" {...layout} ref={formRef}>
