...
const MyFieldAdd = ({ dispatch, visible }) => {
  ...
  const onOk = (form) => {
    form.validateFields().then(values => {
      ...
      let formData = {
        "no": values.no, "field": values.field, "name": values.name,
      ...
      dispatch({
        type: "field/fieldAdd", formData, cb: () => {
          dispatch({ type: "field/rfieldAddVisible", visible: false });
  ...
  const Component = ({ style }) => {
    if (style == "0" || style == "1") {
      ...
      let placeholder = '格式为: {"1": "桥梁", "2": "路面", "3": "隧道"}';
      if (style=="6"){
        placeholder = '格式为: [{"label": "湖北省", "value": "1","children":[{"label":"武汉市","value":"2","children":[{"label":"武昌区","value":"3"},{"label":"汉口区","value":"4"}]}]}]';
      }
      return (
